                      ...
                      >
                        ...
                        <span
                          className={`mt-2 h-px w-10 transition-all duration-300 ${
                            isActive ? "bg-white/28" : "bg-transparent group-hover:bg-white/18"
                          }`}
                        />
              ...
              <div key={activeProject.id} className="animate-fade-up">
                {nextProject ? (
                  <Reveal as="div" className="mb-8 xl:hidden" delay={60} distance={24} blur={6}>
                    <button
                      ...
                      className="group flex w-full items-center justify-between border border-white/10 px-4 py-4 text-left transition-colors hover:bg-white/[0.03]"
                    >
                      <div className="min-w-0">
                        <span className="block text-[11px] uppercase tracking-[0.2em] text-white/30">
                          {ui.nextProject}
                        </span>
                        <span className="mt-2 block truncate text-[24px] leading-[1.02] tracking-[-0.02em] font-light font-serif text-white">
                          {pickLocalized(nextProject.name, lang)}
                        </span>
                      </div>
                      <span className="ml-5 text-[24px] text-white/42 transition-transform duration-300 group-hover:translate-x-1 group-hover:text-white/72">
                        →
                      </span>
                    </button>
                  </Reveal>
                ...
                <Reveal as="section" distance={34} blur={8}>
                  <div className="grid gap-10 border-b border-white/10 pb-10 lg:grid-cols-[minmax(0,1fr)_320px] lg:gap-12">
                      ...
                      <div className="mt-8 flex items-start gap-5 md:gap-7">
                        ...
                        <div className="min-w-0 flex-1">
                          <div className="flex items-start justify-between gap-6">
                            <h2 className="max-w-[760px] text-[clamp(36px,4.6vw,86px)] leading-[0.92] tracking-[-0.03em] font-light font-serif text-white">
                              {projectName}
                            </h2>
                            <span className="hidden pt-3 text-[12px] tracking-[0.24em] text-white/24 md:block">
                    ...
                    <div className="min-w-0 border-t border-white/10 pt-6 lg:border-l lg:border-t-0 lg:pl-8 lg:pt-0">
                      <h3 className="text-[24px] md:text-[34px] leading-none tracking-[-0.02em] font-light font-serif text-white">
                          ...
                          activeProject.contacts.map((contact) => {
                            ...
                            if (!href) {
                              return (
                                ...
                                >
                                  ...
                                  <p className="mt-3 text-[16px] md:text-[18px] leading-relaxed text-white/88">
                                    {contact.value}
                                  </p>
                                </div>
                              );
                            ...
                            return (
                              <a
                                ...
                                className="group py-4 transition-colors hover:text-white"
                              >
                                <p className="text-[11px] uppercase tracking-[0.2em] text-white/30">
                                  {pickLocalized(contact.label, lang)}
                                </p>
                                <div className="mt-3 flex items-center justify-between gap-4">
                                  <p className="text-[16px] md:text-[18px] leading-relaxed text-white/88 transition-colors group-hover:text-white">
                                    {contact.value}
                                  </p>
                                  <span className="text-white/24 transition-all duration-300 group-hover:translate-x-1 group-hover:text-white/58">
                                    →
                                  </span>
                                </div>
                              </a>
                ...
                <Reveal as="section" className="mt-10" delay={120} distance={34} blur={8}>
                  ...
                  {activeProject.gallery.length > 0 ? (
                      ...
                      >
                        {activeProject.gallery.map((image, index) => (
                          <SwiperSlide key={`${activeProject.id}-gallery-${index}`}>
                            <div className="relative aspect-[16/8] w-full overflow-hidden border border-white/10 bg-card">
                              <Image
                                src={image}
                    ...
                    <div className="mt-6 border border-white/10 bg-white/[0.015] py-14 px-6">
                      <p className="max-w-[520px] text-[15px] md:text-[17px] leading-relaxed text-white/56">
                        {ui.noGallery}
                      </p>
                    </div>
                  )}
